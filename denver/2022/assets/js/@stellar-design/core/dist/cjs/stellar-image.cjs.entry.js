...
const Picture = class {
    ...
    renderPicture() {
        if (this.active) {
            return [
                this.sources.map((source) => core.h("source", { srcSet: source.srcset, media: source.media ? source.media : false })),
                core.h("img", { src: this.large, class: "final_image", alt: this.alt })
            ];
        }
    }
    render() {
        if (this.type === "picture") {
            return core.h("figure", { itemtype: "http://schema.org/ImageObject", class: this.active ? 'loaded' : '', onClick: () => { this.zoom && this.zoom.open(); } }, core.h("div", { class: "overlay" }), core.h("picture", null, this.renderPicture()), core.h("div", { class: "placeholder", style: { "background-image": `url(${this.poster})` } }), core.h("stellar-intersection", { element: this.element, in: this.in.bind(this) }));
        }
        else {
            return core.h("stellar-intersection", { element: this.element, in: this.in.bind(this) });
        }
    }
