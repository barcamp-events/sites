...
const index$2 = require('./index-bae84e7c.js');
...
const Item = class {
    constructor(hostRef) {
        ...
        this.mounted = core.createEvent(this, "mounted", 7);
        this.focusChanged = core.createEvent(this, "focusChanged", 7);
        this.blurChanged = core.createEvent(this, "blurChanged", 7);
    }
    componentWillLoad() {
    ...
    render() {
        return (core.h(this.type, { class: "button", type: "button", href: this.href, url: this.href, tabindex: "0", value: this.value, title: this.label, onClick: (e) => this.handleClick(e), onBlur: () => this.handleBlur(), onFocus: () => this.handleFocus() }, core.h("div", { class: "content" }, core.h("slot", null)), (this.selected || this.multiple) && core.h("stellar-asset", { class: this.classes(), name: "checkmark", block: true })));
    }
...
const Layout = class {
    ...
    render() {
        return (core.h("div", { class: "layout" }, core.h("slot", null), core.h("slot", { name: "nav" })));
    }
    get element() { return core.getElement(this); }
...
var rellax = _commonjsHelpers.createCommonjsModule(function (module) {
// ------------------------------------------
// Rellax.js
...
const Select = class {
    ...
    renderLabel() {
        if (this.label) {
            return core.h("div", { class: "label-wrapper" }, core.h("stellar-label", { size: this.size, onClick: () => { this.focusFirstItem(); } }, this.label), this.renderEmptyButton());
        }
    }
    renderErrors() {
        if (this.status && this.status.errors && this.status.errors.length !== 0) {
            return (core.h("p", { class: "validation" }, this.status.errors.map(error => core.h("span", null, error.message))));
        }
    }
    renderEmptyButton() {
        return this.multiple && this.value && this.value.length > 0 && core.h("stellar-button", { class: "clear-button", tag: "button", size: this.size, ghost: true, onClick: (e) => { e.stopPropagation(); this.clearValue(); } }, core.h("stellar-asset", { name: "close" }), this.clear_confirm ? `Clear ${this.value.length} selections?` : `Clear`);
    }
    render() {
        return (core.h("div", { class: "wrapper" }, this.renderLabel(), core.h("div", { class: "select" }, this.loading && core.h("div", { class: "loading" }, core.h("stellar-asset", { name: "loading-spin" }), " ", core.h("p", null, "One sec...")), core.h("button", { type: "button", class: "select-title", onClick: () => this.handleTitleClick(), onFocus: () => this.handleTitleFocus(), onBlur: () => this.handleTitleBlur() }, core.h("stellar-item", { fit: true, wrap: true, "select-title": true, type: "button", value: this.value ? this.value.toString() : "", tabindex: "-1", selectable: false, label: this.language, innerHTML: this.language }), core.h("stellar-asset", { name: "arrow-down" }), this.name && core.h("input", { type: "text", tabindex: "-1", value: this.value, name: this.name.toString(), required: this.required })), this.tooltip && core.h("stellar-tooltip", { align: "bottom-left", onClick: () => this.handleTitleClick() }, this.tooltip), core.h("stellar-blur", { vertical: this.blur, class: "select-list" }, core.h("div", { class: "select-list-header" }, core.h("slot", { name: "header" })), core.h("div", { class: "select-list-body" }, core.h("slot", null)), this.footer && core.h("div", { class: "select-list-footer" }, core.h("slot", { name: "footer" })))), core.h("stellar-label", { size: "small", underneath: true }, this.description), !this.novalidate && this.renderErrors()));
    }
    get element() { return core.getElement(this); }
