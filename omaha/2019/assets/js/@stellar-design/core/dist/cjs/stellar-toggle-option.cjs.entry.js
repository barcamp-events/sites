...
const ToggleOption = class {
    constructor(hostRef) {
        ...
        this.changeToggle = core.createEvent(this, "changeToggle", 7);
    }
    componentWillLoad() {
        this.updateRealType();
    }
    ...
    renderCheckbox() {
        if (this.type === "checkbox" || this.type === "radio") {
            return (core.h("div", { class: this.focused ? "box focused" : "box" }, core.h("div", { class: this.checked ? "indicator active" : "indicator" }, this.type === "checkbox" && core.h("stellar-asset", { name: "checkmark" }))));
        }
    }
    renderRadioBlock() {
        if (this.type === "radio-block") {
            return (core.h("div", { class: "wrapper" }, core.h("div", { class: this.focused ? "box focused" : "box" }, core.h("div", { class: this.checked ? "indicator active" : "indicator" })), core.h("div", { class: "block-content" }, core.h("slot", null))));
        }
    }
    renderCheckBlock() {
        if (this.type === "checkbox-block") {
            return (core.h("div", { class: "wrapper" }, core.h("div", { class: this.focused ? "box focused" : "box" }, core.h("div", { class: this.checked ? "indicator active" : "indicator" }, core.h("stellar-asset", { name: "checkmark", block: true }))), core.h("div", { class: "block-content" }, core.h("slot", null))));
        }
    }
    render() {
        // @ts-ignore
        return core.h("button", { type: "button", onClick: () => this.onClick(), onKeyDown: e => this.onKeyDown(e) }, core.h("input", { type: "hidden", name: `${this.for}[${this.name}]`, value: this.default }), core.h("input", { class: "input", type: this._type, id: `${this.for}_${this.name}_${this.value}`, name: `${this.for}[${this.name}]`, checked: this.checked, value: this.value, required: this.required, onChange: (e) => { e.stopPropagation(); e.preventDefault(); }, onKeyDown: e => this.onKeyDown(e) }), this.renderCheckbox(), ["radio", "checkbox"].indexOf(this.type) !== -1 && core.h("p", null, core.h("slot", null)), this.renderRadioBlock(), this.renderCheckBlock(), ["radio", "checkbox"].indexOf(this.type) === -1 &&
            core.h("stellar-blur", { horizontal: this.blur, class: this.checked ? "status active" : "status" }, core.h("stellar-tag", { size: "small", color: "theme-base5" }, this.selectedCopy)), this.tooltip && core.h("stellar-tooltip", { align: "bottom-left" }, this.tooltip));
    }
